...
//    이 funding안의 entId와 같은 member를 찾아서 찾은 값을 알맞은 DTO에 넣어준다
    @Query("SELECT new com.ssafy.artchain.market.dto.MarketDetailResponseDto(m.id, m.fundingId, f.name, m.status, m.contractAddress,m.transactionHash, m.pieceCount, m.totalCoin, m.coinPerPiece, s.walletAddress, b.walletAddress, cm.name, m.createdAt, m.updatedAt) " +
            "FROM Market m " +
