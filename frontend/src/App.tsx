...
const buyToken = async () => {
  ...
  // 거래 게시판 글 올리기
// 거래 게시판 글 올리기 함수 수정
const addTradePost = async (tokenAddress: string, tokenAmount: string, price: string) => {
  try {
    const marketplaceContract = new web3.eth.Contract(TokenMarketplaceABI.abi, TokenMarketplaceContractAddress);
    ...
    const approveTx = await tokenContract.methods.approve(TokenMarketplaceContractAddress, parseFloat(tokenAmount)).send({ from: account });
    const approveTxReceipt = await web3.eth.getTransactionReceipt(approveTx.transactionHash);
    if (!approveTxReceipt.status) {
    ...
    // 거래 게시글 추가
    await marketplaceContract.methods.addTradePost(tokenAddress, parseFloat(tokenAmount), parseFloat(price)).send({ from: account });
    fetchTokenList();
  } catch (error) {
  ...
  return (
    ...
    <div>
        ...
        <button onClick={() => addTradePost(newTradePostData.tokenAddress, newTradePostData.tokenAmount, newTradePostData.price)}>거래 게시글 추가</button>
